...
public class NewShulkerpackScreenHandler extends ScreenHandler
{
    ...
    private NewShulkerpackScreenHandler(ScreenHandlerType<?> type, int syncId, PlayerInventory playerInventory, int rows) {
        this(type, syncId, playerInventory, new SimpleInventory(9 * rows), rows);
    }


    public static NewShulkerpackScreenHandler createGeneric9x3(int syncId, PlayerInventory playerInventory) {
        return new NewShulkerpackScreenHandler(ScreenHandlerType.GENERIC_9X3, syncId, playerInventory, 3);
    }

    public static NewShulkerpackScreenHandler createGeneric9x3(int syncId, PlayerInventory playerInventory, Inventory inventory) {
        return new NewShulkerpackScreenHandler(ScreenHandlerType.GENERIC_9X3, syncId, playerInventory, inventory, 3);
    }

    public NewShulkerpackScreenHandler(ScreenHandlerType<?> type, int syncId, PlayerInventory playerInventory, Inventory inventory, int rows) {
        super(type, syncId);
    ...
    public void onSlotClick(int slotId, int clickData, SlotActionType actionType, PlayerEntity playerEntity)
    {
        ...
        if (slotId >= 0)  // slotId < 0 are used for networking internals
        {
            ...
            if(stack.getItem() instanceof BackpackItem ||
                    stack.getItem() instanceof ShulkerpackItem ||
    ...
    public ItemStack quickMove(PlayerEntity player, int index) {
        ...
        if(testItem instanceof BackpackItem ||
                testItem instanceof ShulkerpackItem ||
